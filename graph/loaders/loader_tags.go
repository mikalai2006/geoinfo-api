...
type Loaders struct {
	...
	TagoptLoader  *dataloader.Loader[string, []*model.Tagopt]
}
...
func NewLoaders(mongoDB *mongo.Database, repositories *repository.Repositories) *Loaders {
	...
	return &Loaders{
		...
		TagoptLoader:  dataloader.NewBatchedLoader(ur.getTagopt, dataloader.WithClearCacheOnBatch[string, []*model.Tagopt]()),
	}
}
