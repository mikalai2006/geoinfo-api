...
func (u *loaderReader) getTagopt(ctx context.Context, tagoptIds []string) []*dataloader.Result[[]*model.Tagopt] {
	...
	results := make([]*dataloader.Result[[]*model.Tagopt], len(tagoptIds))
	...
	if err != nil {
		for i := 0; i < len(results); i++ {
			results[i] = &dataloader.Result[[]*model.Tagopt]{Error: err}
		}
		return results
	}
	...
	for tagoptID, ix := range keyOrder {
		...
		results[ix] = &dataloader.Result[[]*model.Tagopt]{Data: []*model.Tagopt{}, Error: err}
	}

	return results
}

func GetTagopt(ctx context.Context, itemID string) ([]*model.Tagopt, error) {
	loaders := For(ctx)
